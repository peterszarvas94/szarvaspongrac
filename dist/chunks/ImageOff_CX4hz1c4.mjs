import { createComponent, createAstro, renderComponent, renderTemplate, maybeRenderHead } from './astro/server_DVxX00a6.mjs';
import 'piccolore';
import 'html-escaper';
import { $$ } from './.Layout_BhJMmowD.mjs';

var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
const $$Astro = createAstro("https://szarvaspongrac.hu");
const $$ImageOff = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$ImageOff;
  return renderTemplate`${renderComponent($$result, "Layout", $$, { "iconName": "image-off", ...Astro2.props }, { "default": /* @__PURE__ */ __name(($$result2) => renderTemplate`
	${maybeRenderHead()}<line x1="2" x2="22" y1="2" y2="22"></line>
	<path d="M10.41 10.41a2 2 0 1 1-2.83-2.83"></path>
	<line x1="13.5" x2="6" y1="13.5" y2="21"></line>
	<line x1="18" x2="21" y1="12" y2="15"></line>
	<path d="M3.59 3.59A1.99 1.99 0 0 0 3 5v14a2 2 0 0 0 2 2h14c.55 0 1.052-.22 1.41-.59"></path>
	<path d="M21 15V5a2 2 0 0 0-2-2H9"></path>
`, "default") })}`;
}, "/Users/szarvaspeter/projects/szarvaspongrac/node_modules/lucide-astro/dist/ImageOff.astro", void 0);

export { $$ImageOff };
