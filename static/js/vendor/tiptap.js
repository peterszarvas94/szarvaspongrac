...
// scripts/tiptap-figure.js
var inputRegex4 = /(?:^|\s)(!\[(.+|:?)]\((\S+)(?:(?:\s+)["'](\S+)["'])?\))$/;
var resizePlugin = new Plugin({
  props: {
    handleDOMEvents: {
    ...
    return [
      {
        ...
        getAttrs: (node) => {
          ...
          if (!img) return false;
          const style2 = figure.getAttribute("style") || "";
          const alignMatch = style2.match(/text-align:\s*(left|center|right|justify)/);
          return {
